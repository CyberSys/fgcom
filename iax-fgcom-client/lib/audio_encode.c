...
static SpeexPreprocessState *s_speex_state = NULL;
...
static void set_speex_filters()
{
	...
	speex_preprocess_ctl(s_speex_state, SPEEX_PREPROCESS_SET_VAD, &i);
	i = (iaxci_filters & IAXC_FILTER_AGC) ? 1 : 0;
	speex_preprocess_ctl(s_speex_state, SPEEX_PREPROCESS_SET_AGC, &i);
	i = (iaxci_filters & IAXC_FILTER_DENOISE) ? 1 : 0;
	speex_preprocess_ctl(s_speex_state, SPEEX_PREPROCESS_SET_DENOISE, &i);

	/* make vad more sensitive */
	i = 30;
	speex_preprocess_ctl(s_speex_state, SPEEX_PREPROCESS_SET_PROB_START, &i);
	i = 7;
	speex_preprocess_ctl(s_speex_state, SPEEX_PREPROCESS_SET_PROB_CONTINUE, &i);
}
...
static int input_postprocess(void *audio, int len, int rate)
{
	...
	if ( !s_speex_state || speex_state_size != len || speex_state_rate != rate )
	{
		if (s_speex_state)
			speex_preprocess_state_destroy(s_speex_state);
		s_speex_state = speex_preprocess_state_init(len,rate);
	...
	if ( (iaxci_filters & (IAXC_FILTER_DENOISE | IAXC_FILTER_AGC)) ||
		...
		silent = !speex_preprocess(s_speex_state, (spx_int16_t *)audio, NULL);
	...
	if ( !silent &&
			...
			s_speex_state->speech_prob > 0.20f )
	{
		static int i = 0;
		...
		{
			/* 
			 * Modification by Charles INGELS, 11/12/2008.
			 * It seems that the field s_speex_state->loudness2 is not defined
			 * any longer in the SpeexPreprocessState structure ! So it cannot 
			 * be used here.
			 *
			 * const float loudness = s_speex_state->loudness2;
			 */
			const float loudness = s_speex_state->loudness;
