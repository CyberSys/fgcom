...
gboolean
net_open (gchar * hostname, gint port)
{
	gint rv;
	gchar buffer[1024];
	guint n;
	...
	/* Create the socket */
	config.fg_socket =
		gnet_udp_socket_new_full (config.fg_addr, config.fg_port);
	g_assert (config.fg_socket != NULL);
}

gboolean
net_block_read (gchar * buffer)
{
	gint n = -1;

	/* Receive packet */
	n = gnet_udp_socket_receive (config.fg_socket, buffer,
						    FGCOM_UDP_MAX_BUFFER, NULL);
	if (n <= 0)
	{
